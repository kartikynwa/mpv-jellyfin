...
local function ticks_to_seconds(ticks) return ticks / (1000 * 10000) end
...
local function send_request(method, url, opts)
    if #api_key > 0 then
        ...
        if request then return utils.parse_json(request.stdout) end
    end
    return nil
end
...
local function sessions_playing()
    ...
    if payload ~= nil then send_request_async("POST", url, { body = payload }) end
end
...
local function sessions_stopped()
    ...
    if payload ~= nil then send_request("POST", url, { body = payload }) end
end
